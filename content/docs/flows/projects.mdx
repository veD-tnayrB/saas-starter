...
- **Kysely + raw SQL** – Type-safe query builder and SQL for managing projects, members, and invitations
...
```
emails/
└── project-invitation-email.tsx – React Email template for project invitations
```
...
6. **Email Sending**:
   - Renders invitation email template using `@react-email/render`
   - Template includes project name, inviter name, role, and accept URL
   - Sends email via Resend API with HTML and plain text versions
   - Accept URL format: `/accept-invitation?token={token}`
