---
title: Billing and Settings Flow
...
- `app/(protected)/dashboard/[projectId]/settings/page.tsx` – Consumes subscription data to display billing status in the dashboard.
...
1. The UI triggers `openCustomerPortal` action (usually from the project settings page).
...
5. On return (configured to `/project/[projectId]/settings`), the dashboard reloads data to show any changes made in the portal.
...
```mermaid
flowchart TD
  A[User clicks Upgrade/Manage] --> B[Server Action (generateUserStripe or openCustomerPortal)]
  B --> C[BillingService]
  C --> D[Stripe SDK]
