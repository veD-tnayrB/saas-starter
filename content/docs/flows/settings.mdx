...
The settings flow allows authenticated users to update personal information (e.g., display name) and manage destructive actions such as account deletion. `app/(protected)/dashboard/settings/page.tsx` renders the page with form components that rely on server actions (`actions/update-user-name.ts`) and services (`services/auth/user.ts`). After profile changes, the flow revalidates cached session data and layout routes so the updated name propagates across the application.
...
- `app/(protected)/dashboard/settings/page.tsx` – Server component fetching the current user and rendering forms.
- `actions/update-user-name.ts` – Server action for name updates (`"use server"`); revalidates `/dashboard/settings`, `/dashboard`, and `/`.
...
5. The action calls `revalidatePath("/dashboard/settings", "layout")`, `revalidatePath("/dashboard", "layout")`, and `revalidatePath("/", "layout")` to refresh cached layouts and server components that display user data.
...
flowchart TD
  ...
  F --> G[(PostgreSQL)]
  D --> H[revalidatePath /dashboard/settings,/dashboard,/]
