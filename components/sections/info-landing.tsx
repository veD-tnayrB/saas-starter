...
import { IInfoLdg } from "@/types";

import { cn } from "@/lib/utils";
import { Icons } from "@/components/shared/icons";
import MaxWidthWrapper from "@/components/shared/max-width-wrapper";

interface IInfoLandingProps {
  data: IInfoLdg;
  reverse?: boolean;
}
...
export default function InfoLanding({
  ...
  const infoItems = data.list.map(function renderInfoItem(item) {
    ...
    <div className="py-10 sm:py-20">
      <MaxWidthWrapper className="grid gap-10 px-2.5 lg:grid-cols-2 lg:items-center lg:px-7">
        <div className={cn(reverse ? "lg:order-2" : "lg:order-1")}>
          ...
          <dl className="mt-6 space-y-4 leading-7">{infoItems}</dl>
        </div>
        <div
          className={cn(
